...
class Dho(BasePinns):
    def __init__(self, config):
        ...
        params = config.system_dho.system_params  # Extract system parameters
        ...
        x0 = params['x0']
        v0 = params['v0']
        self.x0 = torch.tensor([[x0]], dtype=torch.float32).to(config.device)
        self.v0 = torch.tensor([[v0]], dtype=torch.float32).to(config.device)
        self.T = params['T']

        # N_col = 10000  # number of collocation points
        # lhs_sample = lhs(n=1, samples=N_col)  # 1D LHS
        # t_lhs = torch.tensor(lhs_sample * self.T, dtype=torch.float32).to(config.device)
        # self.t = t_lhs.reshape(-1, 1)  # shape (10000, 1)
    ...
    def f_loss(self, corrector=None):
        ...
        # IC loss
        ic_loss_x = torch.mean((y[0] - self.x0)**2)
        ic_loss_v = torch.mean((dy[0] - self.v0)**2)
