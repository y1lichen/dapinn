...
# 確保輸出根目錄存在
mkdir -p "${WORKDIR}/results/pedagogical"
...
for SIZE in "${SAMPLE_SIZES[@]}"; do
    ...
    for ((i=1; i<=NUM_TRIALS; i++)); do
        ...
        SAVE_SUBDIR="results/pedagogical/size_${SIZE}/trial_${i}_seed_${CURRENT_SEED}"
...
echo " Results are stored in results/pedagogical/size_N/trial_i_seed_S "
echo "============================================================"