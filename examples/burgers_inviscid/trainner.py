...
import copy
from .models import Burgers, Corrector
from .utils import generate_inviscid_burgers_dataset, generate_viscous_burgers_dataset
from .eval import evaluate_pretrained_pinns

from torch.utils.data import DataLoader, TensorDataset
from dapinns.samplers import TimeSpaceUniformSampler, TimeSpaceRandomSampler
...
def pretrain(config: ml_collections.ConfigDict, workdir:str):
    ...
    # System parameters
    params = config.system_inviscid_burgers.system_params
    T = params['T']
    n_x = params['n_x']
    n_t = params['n_t']

    # Data generation
    x, t, u = generate_inviscid_burgers_dataset(params, T=T, n_x=n_x, n_t=n_t)
    
    # sample
    sampler = TimeSpaceUniformSampler(sample_size=config.pretrained_sample_size)
    xyt_train, u_train = sampler.generate_data(x, t, u)
    xyt_train, u_train = xyt_train.to(config.device), u_train.to(config.device)
    ...
    for epoch in range(max_epochs):
        ...
        # training
        u_loss = model.u_loss(xyt_train, u_train)
        f_loss = model.f_loss()
        ...
        if (epoch + 1) % 1000 == 0:
            print(f"Learning rate: {model.scheduler.get_last_lr()[0]:.4}, Epoch: {epoch + 1}, u_loss: {u_w*u_loss.item():.4e}, f_loss: {f_w*f_loss.item():.4e}, Loss: {total_loss.item():.4e}, Val_loss: {val_f_loss.item():.4e}, Best epoch: {best_epoch}")
            
        # monitor val f loss
        model.scheduler.step(val_f_loss.item())
        is_best = (epoch + 1) > warmup_epochs and val_f_loss.item() < best_val_f_loss
        if is_best:
            best_val_f_loss = val_f_loss.item()
            best_epoch = epoch + 1
            save_checkpoint(state, save_dir, epoch + 1, keep=keep, verbose=False, name=config.pretrained_model_name)

        # Periodic save
        if (epoch + 1) % save_interval == 0:
            save_checkpoint(state, save_dir, epoch + 1, keep=keep)
    ...
    # save_checkpoint(final_model_state, save_dir, epoch + 1, keep=config.saving.keep)
...
def finetune(config: ml_collections.ConfigDict, workdir: str):
    ...
    T, n_x, n_t = params['T'], params['n_x'], params['n_t']
    ...
    # Generate dataset
    x, t, u, _, _ = generate_viscous_burgers_dataset(params, T=T, n_x=n_x, n_t=n_t)

    # Sample data
    sampler = TimeSpaceRandomSampler(config, sample_size=config.finetune_sample_size)
    xyt_train, u_train = sampler.generate_data(x, t, u)
    xyt_train, u_train = xyt_train.to(config.device), u_train.to(config.device)


    # Initialize model and corrector
    model = Burgers(config).to(config.device)
    ...
    model_steps = 1
    corrector_steps = 2
    total_steps = corrector_steps + model_steps
    
    for epoch in range(max_epochs):
        # step_in_cycle = epoch % total_steps
        # if step_in_cycle < model_steps:
        #     model.optimizer.zero_grad()
        #     u_loss = model.u_loss(xyt_train, u_train)
        ...
        # # Early stopping and LR decay
        # if best_epoch is not None and config.saving.decay_step and config.saving.early_stopping_patience:
        #     if step_epoch is None:
        #         step_epoch = best_epoch  # Initialize at first best epoch

        #     if (epoch + 1 - step_epoch) >= config.saving.decay_step:
        #         model.scheduler.step()
        #         corrector.scheduler.step()
        #         step_epoch = epoch + 1  # Update step_epoch to mark when LR was last decayed
