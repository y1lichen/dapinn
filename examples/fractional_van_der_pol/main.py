...
from examples.fractional_van_der_pol import trainner, eval

FLAGS = flags.FLAGS

config_flags.DEFINE_config_file(
    "config",
    "examples/fractional_van_der_pol/configs/default.py",
...
flags.DEFINE_float("alpha", None, "Fractional order (Overrides config)")
flags.DEFINE_float("mu", None, "Damping parameter (Overrides config)")
...
def main(argv):
    ...
    if FLAGS.alpha is not None:
        config.system_fvdp.system_params["alpha"] = FLAGS.alpha
    if FLAGS.mu is not None:
        config.system_fvdp.system_params["mu"] = FLAGS.mu
