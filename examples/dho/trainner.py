...
from .models import Dho, Corrector
from .utils import generate_uho_dataset, generate_dho_dataset
...
def pretrain(config: ml_collections.ConfigDict, workdir:str):
    ...
    for epoch in range(max_epochs):
        ...
        #--------------------------------------
        # Logging & checkpoint every 10 epochs
        if (epoch + 1) % 100 == 0:
            wandb.log({
                "u_loss": u_loss.item(),
                "f_loss": f_loss.item(),
...
def finetune(config: ml_collections.ConfigDict, workdir: str):
    ...
    for epoch in range(max_epochs):
        ...
        if is_best:
            ...
            # best_corrector_state = corrector_state.copy()
