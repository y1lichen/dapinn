...
def get_config():
    ...
    config.finetune_sample_size = 100
    ...
    # Optimizer
    def optimizer_config(lr, optimizer, scheduler, gamma=None):
        return ml_collections.ConfigDict({
            "lr": lr,
            "optimizer": optimizer,
            "scheduler": scheduler,
            "gamma": gamma if scheduler == "Exp" else None,
        })
    ...
    config.finetune_pinns_optim = optimizer_config(lr=1e-3, optimizer="Adam", scheduler="Reduce")
    config.finetune_correction_optim = optimizer_config(lr=1e-3, optimizer="Adam", scheduler="Reduce")
    ...
    config.pretraining = training_config(max_epochs=100000, u_w=1.0, f_w=1e-6)
    config.finetuning = training_config(max_epochs=60000, u_w=1.0, f_w=1e-6, alt_steps=50)  
