...
from .models import FractionalVanderPol, Corrector
from .utils import generate_fractional_vanderpol_dataset
...
def pretrain(config: ml_collections.ConfigDict, workdir: str):
    ...
    # Generate dataset
    t, x1, x2, _ = generate_fractional_vanderpol_dataset(
        params, T=T, x1_0=x1_0, x2_0=x2_0, n_t=n_t, alpha=alpha
    )

    # Sample
    sampler = UniformSampler(sample_size=config.pretrained_sample_size)
    t_train, x1_train = sampler.generate_data(t, x1)
    _, x2_train = sampler.generate_data(t, x2)
    
    t_train = t_train.to(config.device)
    x1_train = x1_train.to(config.device)
    x2_train = x2_train.to(config.device)
    ...
    for epoch in range(max_epochs):
        ...
        # Training
        u_loss = model.u_loss(t_train, x1_train, x2_train)
        f_loss = model.f_loss()
...
def finetune(config: ml_collections.ConfigDict, workdir: str):
    ...
    early_stopping_patience = config.saving.early_stopping_patience
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % 1000 == 0:
            print(f"Epoch: {epoch + 1}, u_loss: {u_w*u_loss.item():.4e}, f_loss: {f_w*f_loss.item():.4e}, Loss: {total_loss.item():.4e}, Best epoch: {best_epoch}")
    ...
    lbfgs_optimizer = torch.optim.LBFGS(lbfgs_params, max_iter=500, tolerance_grad=1e-5, tolerance_change=1e-7)
