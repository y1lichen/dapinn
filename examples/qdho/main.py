...
from examples.qdho import sr
from examples.qdho import trainner, eval
...
def setup_wandb_config(config):
    return ml_collections.ConfigDict({
        "project": "DAPINNs-QDHO-Scarce-Level",
        "name": config.name,
...
def main(argv):
    ...
    config_flags.DEFINE_config_file(
        "config",
        f"examples/qdho/configs/{config_name}", # default.py, default_noise.py, fourier_emb.py
    ...
    if FLAGS.noise is not None:
        config.system_qdho.system_params["noise"] = flags.FLAGS.noise
