...
def evaluate_finetune_by_seeds(config, workdir, models, case):
    ...
    for scarce_level in level_ints:
        ...
        error_plot_path = os.path.join(finetuned_scarce_level_dir, f"{scarce_level}pts-{case.lower()}-seeds-l2error-mean-std.png")
...
def main(argv):
    ...
    noise_models, SUBFOLDERS = load_seeds_models(config, FLAGS.workdir, case="DHO", target="noise_seeds", SUBFOLDERS=SUBFOLDERS)
    evaluate_finetune_by_seeds(config, FLAGS.workdir, noise_models, case="DHO")
    noise_correctors, noise_correctors_inputs, SUBFOLDERS = load_seeds_correctors(config, FLAGS.workdir, case="DHO", target="noise_seeds", SUBFOLDERS=SUBFOLDERS)
    evaluate_corrector_by_seeds(config, FLAGS.workdir, noise_correctors, noise_correctors_inputs, case="DHO")
