...
from examples.qdho.utils import generate_uho_dataset, generate_qdho_dataset
...
def _get_qdho_test(config: ml_collections.ConfigDict, workdir: str):
    ...
    # Generate dataset
    x, y, sol = generate_qdho_dataset(params, T=T, x0=x0, v0=v0, n_t=n_t) # dho
...
def _get_index(config: ml_collections.ConfigDict):
...
def evaluate_finetuned_pinns(config: ml_collections.ConfigDict, workdir: str):
    ...
    # Get mearsurements indices
    y_train, indices =_get_index(config)

    # Plotting
    plt.figure(figsize=(8, 5))
    plt.plot(t, ground_truth, label="Reference")
    plt.plot(t, y_preds, label="DAPINNs Predictions", color='orange', linestyle="--")
    plt.scatter(t[indices], y_train.cpu().detach().numpy().ravel(), color='green', label=f"Measurements ({config.finetune_sample_size} pts)", s=10)
    plt.xlabel("Time (t)")
    plt.ylabel("Displacement x(t)")
    plt.legend()
    plt.title("DAPINNs: Prediction on Quadratic Damped Harmonic Oscillator")
    plt.savefig(finetuned_dir + os.sep + "finetune.png", dpi=300)
    print(f"Figure finetune.png saved to {finetuned_dir}")
    print("-"*80)
    # plt.figure(figsize=(8, 5))
    # plt.plot(t, ground_truth, label="Reference")
    # plt.plot(t, y_preds, label="PINNs Predictions", color='orange', linestyle="--")
    # plt.scatter(t[indices], y_train.cpu().detach().numpy().ravel(), color='green', label="Measurements", s=10)
    # plt.xlabel("Time (t)")
    # plt.ylabel("Displacement x(t)")
    # plt.legend()
    # plt.title("Misspecified PINNs")
    # plt.savefig(finetuned_dir + os.sep + "Misspecified-PINNs-15.png", dpi=300)
    # print(f"Figure finetune.png saved to {finetuned_dir}")
    # print("-"*80)
...
def evaluate_corrector(config: ml_collections.ConfigDict, workdir: str):
    ...
    s = s.cpu().detach().numpy().ravel()[::10] # align

    # Get mearsurements indices
    _, indices =_get_index(config)
    ...
    plt.scatter(t[indices], ground_truth[indices], color='green', label=f"Measurements ({config.finetune_sample_size} pts)", s=10)
    plt.xlabel("Time (t)")
    plt.ylabel(r'Correction $s_{\psi}$')
    plt.legend()
    plt.title("Predicted Correction (Quadratic Damping Term)")
    plt.savefig(corrector_dir + os.sep + "corrector.png", dpi=300)
    print(f"Figure corrector.png saved to {corrector_dir}")
    
def evaluate_corrector_by_finetune_u_inputs(config, workdir):
    # this function can be used to different resolution
    device = config.device
    n_t = 100000
    config.system_qdho.system_params.n_t = n_t
    x_test, y_test, sol = _get_qdho_test(config, workdir)
    t = sol.t
    dx = sol.y[1]
    c = config.system_qdho.system_params['c']
    ...
    collocations = torch.linspace(0, 1, n_t).reshape(-1, 1).to(device)  # Collocation points
    collocations.requires_grad = True
    model = Qdho(config).to(device)
    finetuned_dir = os.path.join(workdir, config.saving.save_dir, config.saving.finetune_path)
    finetuned_model_dir = os.path.join(finetuned_dir, config.finetuned_model_name)
    model.load_finetuned_model(finetuned_model_dir)
    model.eval()
    y = model(collocations)
    dy = torch.autograd.grad(y, collocations, torch.ones_like(y).to(device), create_graph=True, retain_graph=True)[0]
    dy2 = torch.autograd.grad(dy, collocations, torch.ones_like(dy).to(device), create_graph=True)[0]
    ...
    s = s.cpu().detach().numpy().ravel() # align
    collocations = collocations.cpu().detach().numpy()
    # Plotting
    plt.figure(figsize=(8, 5))
    plt.plot(t, s, label="Predicted Correction", color='orange', linestyle="--")
    plt.plot(t, ground_truth, label="Ground truth misspecified term")
    ...
    plt.title("Predicted Correction (Quadratic Damping Term)")
    plt.savefig(corrector_dir + os.sep + "corrector.png", dpi=300)
    print(f"Figure test.png saved to {corrector_dir}")
...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    evaluate_corrector_by_finetune_u_inputs(config, workdir)