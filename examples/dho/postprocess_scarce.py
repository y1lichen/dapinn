...
from examples.dho.utils import generate_dho_dataset
...
# def load_finetuned_models(config, workdir: str, case: str, target: str):
...
#         finetuned_model_dir = os.path.join(workdir, target, case, str(sf), lower_case, "finetuned", "best_f_loss_model.pt")
...
#     if case == "DHO":
#         case_dir = os.path.join(postprocess_dir, "DHO")
...
# def load_correctors_models(config, workdir: str, case: str, target: str):
#     lower_case = case.lower()
#     if target == "Scarce-level-result":
#         SUBFOLDERS = [1, 3, 5, 10, 15, 100, 10000]   
#     elif target == "Noise-level-result":
...
def load_seeds_models(config, workdir: str, case: str, target: str, SUBFOLDERS=None):
    ...
    if SUBFOLDERS is None:
        SUBFOLDERS = [5, 10, 15, 30, 60, 100, 1000]
...
def evaluate_finetune_by_seeds(config, workdir, models, case):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(t, ground_truth, label="Reference", linewidth=2, color="dimgray")
    plt.plot(t, y_plot_dict[10], label=f"5 pts (L2: {y_error_plot_dict[10]:.2e})", linestyle="--", alpha=0.6, color='red')
...
def evaluate_corrector_by_seeds(config, workdir, models, inputs, case):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(t, ground_truth, label="Reference", linewidth=2, color="dimgray")
    plt.plot(t, y_plot_dict[l1], label=f"{l1} pts (L2: {y_error_plot_dict[l1]:.2e})", linestyle="--", alpha=0.6, color='red')
    plt.plot(t, y_plot_dict[l2], label=f"{l2} pts (L2: {y_error_plot_dict[l2]:.2e})", linestyle="--", alpha=0.6, color='blue')
    plt.fill_between(t, y_plot_dict[l1] - 2*y_std_dict[l1], y_mean + 2*y_std_dict[l1], label=f'±2 Std Dev ({l1} pts)', alpha=0.2, color='red')
    plt.fill_between(t, y_plot_dict[l2] - 2*y_std_dict[l2], y_mean + 2*y_std_dict[l2], label=f'±2 Std Dev ({l2} pts)', alpha=0.2, color='blue')
    plt.xlabel("Time (t)")
    plt.ylabel("Displacement x(t)")
    plt.title(f"Corrector Prediction (mean) on {case_title}")
    plt.legend(loc="best", frameon=True)
    plt.savefig(os.path.join(corrector_dir, f"comparison-{case.lower()}-{l1}-vs-{l2}-pts-corrector-scarce-level.png"), dpi=300)
...
def main(argv):
    ...
    config_flags.DEFINE_config_file(
        "config",
        "examples/dho/configs/default.py",
    ...
    scarce_models, SUBFOLDERS = load_seeds_models(config, FLAGS.workdir, case="DHO", target="scarce_seeds", SUBFOLDERS=SUBFOLDERS)
    evaluate_finetune_by_seeds(config, FLAGS.workdir, scarce_models, case="DHO")
    scarce_correctors, scarce_correctors_inputs, SUBFOLDERS = load_seeds_correctors(config, FLAGS.workdir, case="DHO", target="scarce_seeds", SUBFOLDERS=SUBFOLDERS)
    evaluate_corrector_by_seeds(config, FLAGS.workdir, scarce_correctors, scarce_correctors_inputs, case="DHO")
    
    #---------------- noise levels -----------------
    # noise_models, SUBFOLDERS = load_finetuned_models(config, FLAGS.workdir, case="DHO", target="Noise-level-result")
    # evaluate_finetune_by_noise_levels(config, FLAGS.workdir, noise_models, case="DHO")
    # noise_correctors, noise_correctors_inputs, SUBFOLDERS = load_correctors_models(config, FLAGS.workdir, case="DHO", target="Noise-level-result")
    # evaluate_corrector_by_noise_levels(config, FLAGS.workdir, scarce_correctors, scarce_correctors_inputs, case="DHO")
