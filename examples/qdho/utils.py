...
# Quadratic Damped Harmonic Oscillator
def generate_qdho_dataset(params, T=1.0, x0=1, v0=0, n_t=1000):

    m = params['m']
    k = params['k']
    c = params['c']

    def quadratic_damped_oscillator(t, y):
        x, v = y
        dxdt = v
        dvdt = -(c / m) * v * abs(v) - (k / m) * x
        return [dxdt, dvdt]
    
    t_vals = torch.linspace(0, T, n_t).reshape(-1,1) # x
    sol = solve_ivp(quadratic_damped_oscillator, [t_vals[0], t_vals[-1]], [x0, v0], t_eval=t_vals.ravel())
...
if __name__ == '__main__':
    ...
    f_x, f_y, _ = generate_qdho_dataset(params, T=1.0, x0=1.0, v0=0.0, n_t=1000)
